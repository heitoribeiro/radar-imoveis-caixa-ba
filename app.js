...
const DATA_URL = new URL('data/imoveis-ba.json', document.baseURI).toString();
...
const els = {
  ...
  sourceLine: $('#sourceLine'), searchInput: $('#searchInput'), citySelect: $('#citySelect'), neighborhoodSelect: $('#neighborhoodSelect'),
...
const state = {
  ...
  generatedAt: null, fetchedAt: null, loading: false, workerRegistration: null, optionsReady: false,
};
...
function formatGeneratedDate(value) { return value ? formatDate(value) : null; }
...
function parseDescriptionFallback(p) {
  ...
  const getFloat = (re) => { const m = text.match(re); return m ? Number(String(m[1]).replace(',', '.')) : null; };
...
function reconcileSeen(properties) {
  ...
  for (const property of properties) {
    ...
    const id = property.numeroImovel; const firstSeenGlobal = property.firstSeen ? `${property.firstSeen}T12:00:00-03:00` : null;
    const locallyUnseen = !state.seenAt[id]; if (locallyUnseen) state.seenAt[id] = firstSeenGlobal || now;
    property.detectedAt = firstSeenGlobal || state.seenAt[id]; property.isNew = Boolean(property.newOnLatestUpdate) || (hadBaseline && locallyUnseen);
    if (property.isNew) state.newIds.add(id);
  }
...
function optionsHtml(values, allLabel) { return `<option value="">${escapeHtml(allLabel)}</option>${values.map(v => `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`).join('')}`; }

function populatePrimaryOptions() {
  const cities = uniqueSorted(state.properties.map(p => p.cidade));
...
function populateNeighborhoods(preserve = true) {
  const current = preserve ? els.neighborhoodSelect.value : '';
  const city = els.citySelect.value;
  const neighborhoods = uniqueSorted(state.properties.filter(p => !city || p.cidade === city).map(p => p.bairro));
...
function getFilters() {
  return {
    q: els.searchInput.value.trim(), city: els.citySelect.value, neighborhood: els.neighborhoodSelect.value, type: els.typeSelect.value,
...
function filterLabels(f) {
  const items = [];
  if (f.q) items.push(`Busca: ${f.q}`); if (f.city) items.push(f.city); if (f.neighborhood) items.push(f.neighborhood); if (f.type) items.push(f.type);
...
function restoreFiltersFromUrl() {
  ...
  els.searchInput.value = p.get('q') || ''; els.citySelect.value = p.get('city') || ''; populateNeighborhoods(false); els.neighborhoodSelect.value = p.get('bairro') || '';
...
function applyFilters({ sync = true } = {}) {
  ...
  const filtered = state.properties.filter((p) => {
    ...
    if (f.onlyFavorites && !state.favorites.has(p.numeroImovel)) return false;
    if (search) {
      const haystack = normalize([p.numeroImovel, p.cidade, p.bairro, p.endereco, p.modalidade, p.descricao, p.financiamento, p.tipoImovel].join(' '));
...
function propertyCard(p) {
  const favorite = state.favorites.has(p.numeroImovel); const modality = p.modalidade || 'Modalidade não informada';
  ...
  const mapQuery = encodeURIComponent([p.endereco, p.bairro, p.cidade, 'BA'].filter(Boolean).join(', '));
  return `<article class="property-card" data-id="${escapeHtml(p.numeroImovel)}">
    <div class="card-top">
      <div class="card-badges"><span class="badge badge--modality" title="${escapeHtml(modality)}">${escapeHtml(modality)}</span><div class="card-badge-actions">${p.isNew ? '<span class="badge badge--new">Novo</span>' : ''}<button class="favorite-button ${favorite ? 'is-favorite' : ''}" data-favorite="${escapeHtml(p.numeroImovel)}" type="button" title="${favorite ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}" aria-label="Favorito">♥</button></div></div>
      <div class="card-city">${escapeHtml(p.cidade)} • BA</div><h3 class="card-neighborhood">${escapeHtml(p.bairro || 'Bairro não informado')}</h3>
      <div class="property-features">${featureChips(p)}</div><span class="price-label">Preço de venda</span>
      <div class="price-row"><span class="price">${formatMoney(p.preco)}</span><span class="discount">${escapeHtml(discount)}</span></div>
      ${Number.isFinite(p.precoM2) ? `<div class="sqm-price">${formatMoney(p.precoM2)} / m² privativo</div>` : ''}
    </div>
    <div class="card-body">
      <div class="info-grid">
        <div class="info-item"><span>Avaliação</span><strong>${escapeHtml(formatMoney(p.valorAvaliacao))}</strong></div><div class="info-item"><span>Nº do imóvel</span><strong>${escapeHtml(p.numeroImovel)}</strong></div>
        <div class="info-item"><span>Financiamento</span><strong>${escapeHtml(p.financiamento || 'Consulte')}</strong></div><div class="info-item"><span>Área total</span><strong>${Number.isFinite(p.areaTotal) ? `${escapeHtml(formatNumber(p.areaTotal, 1))} m²` : '—'}</strong></div>
      </div>
      <div class="address">${escapeHtml(p.endereco || 'Endereço disponível na página oficial.')}</div><div class="description">${escapeHtml(p.descricao || 'Consulte a descrição completa no anúncio oficial da CAIXA.')}</div>
      <div class="card-footer"><span class="detected-date">${escapeHtml(detected)}</span><div class="card-links"><a class="card-link card-link--secondary" href="https://www.google.com/maps/search/?api=1&query=${mapQuery}" target="_blank" rel="noopener noreferrer">Mapa ↗</a><a class="card-link" href="${escapeHtml(p.link)}" target="_blank" rel="noopener noreferrer">Ver na CAIXA ↗</a></div></div>
    </div></article>`;
}
...
function renderMeta() {
  const generated = formatGeneratedDate(state.generatedAt); const f = getFilters(); const parts = [];
  if (f.city) parts.push(f.city); if (f.type) parts.push(f.type); parts.push(`${numberFormat.format(state.filtered.length)} resultado${state.filtered.length === 1 ? '' : 's'}`); if (generated) parts.push(`lista CAIXA de ${generated}`);
...
function updateSourceLine() {
  const generated = formatGeneratedDate(state.generatedAt); const fetched = state.fetchedAt ? formatDate(state.fetchedAt) : null;
  els.sourceLine.textContent = generated ? `Fonte oficial CAIXA • lista gerada em ${generated} • ${numberFormat.format(state.properties.length)} imóveis carregados` : `Fonte oficial CAIXA • ${numberFormat.format(state.properties.length)} imóveis carregados${fetched ? ` • consultado em ${fetched}` : ''}`;
}
...
function clearFilters() {
  els.searchInput.value = ''; els.citySelect.value = ''; populateNeighborhoods(false); els.typeSelect.value = ''; els.sortSelect.value = 'recent'; els.minPriceInput.value = ''; els.maxPriceInput.value = ''; els.minDiscountInput.value = '';
...
function exportCsv() {
  ...
  const cols = [
    ['Nº imóvel','numeroImovel'],['Cidade','cidade'],['Bairro','bairro'],['Tipo','tipoImovel'],['Endereço','endereco'],['Preço','preco'],['Avaliação','valorAvaliacao'],['Desconto %','desconto'],['Quartos','quartos'],['WC','banheiros'],['Vagas','vagas'],['Área privativa m²','areaPrivativa'],['Área total m²','areaTotal'],['Preço/m²','precoM2'],['Financiamento','financiamento'],['Modalidade','modalidade'],['Primeira detecção','firstSeen'],['Link CAIXA','link']
  ];
  const rows = [cols.map(c => c[0]).join(';'), ...state.filtered.map(p => cols.map(([,k]) => csvEscape(p[k])).join(';'))];
  const blob = new Blob(['\ufeff' + rows.join('\n')], { type: 'text/csv;charset=utf-8' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `radar-caixa-ba-${new Date().toISOString().slice(0,10)}.csv`; a.click(); URL.revokeObjectURL(a.href);
...
async function shareSearch() {
  ...
  try {
    if (navigator.share) await navigator.share({ title: 'Radar de Imóveis CAIXA - Bahia', text: 'Veja esta pesquisa de imóveis da CAIXA:', url });
    else { await navigator.clipboard.writeText(url); showToast('Link da pesquisa copiado.'); }
...
function profileSummary(profile) { const labels = filterLabels({ ...profile, q: '', sort: 'recent', onlyNew: false, onlyFavorites: false }); return labels.length ? labels.slice(0, 4).join(' • ') + (labels.length > 4 ? ` • +${labels.length - 4}` : '') : 'toda a Bahia'; }
function currentFiltersAsAlertProfile() { const f = getFilters(); return { city:f.city, neighborhood:f.neighborhood, type:f.type, minPrice:f.minPrice, maxPrice:f.maxPrice, minDiscount:f.minDiscount, bedrooms:f.bedrooms, bathrooms:f.bathrooms, parking:f.parking, financing:f.financing, modality:f.modality, minArea:f.minArea, maxArea:f.maxArea }; }

async function syncWorkerState() {
  if (!state.workerRegistration) await registerServiceWorker();
  workerMessage({ type: 'SYNC_CURRENT', ids: state.properties.map(p => p.numeroImovel), alertProfile: alertProfile() });
}
...
async function fetchProperties(force = false) {
  ...
  try {
    ...
    state.properties = Array.isArray(payload.properties) ? payload.properties : []; state.generatedAt = payload.generatedAt || null; state.fetchedAt = payload.fetchedAt || new Date().toISOString(); reconcileSeen(state.properties);
    populatePrimaryOptions(); populateNeighborhoods(false); restoreFiltersFromUrl(); applyFilters({ sync: false }); updateSourceLine(); els.errorState.classList.add('hidden'); await notifyNewPropertiesIfNeeded(); await syncWorkerState(); if (force) showToast('Dados publicados verificados.');
  } catch (error) {
    console.error(error); els.errorText.textContent = error instanceof Error ? error.message : 'Tente novamente em instantes.'; els.errorState.classList.remove('hidden'); els.cardsGrid.classList.add('hidden'); els.emptyState.classList.add('hidden'); els.sourceLine.textContent = 'Não foi possível consultar a lista oficial agora.';
